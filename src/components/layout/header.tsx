...
export const Header = () => {
  ...
  const academyLinks = [
    ...
    { href: "#parents", label: t('parents', 'nav_academy') },
    { href: "#awards", label: t('awards', 'nav_academy') },
    { href: "#recruiters", label: t('recruiters', 'nav_academy') },
    { href: "#student-life", label: t('student_life', 'nav_academy') },
  ];

  const trainingLinks = [
    { href: "#program", label: t('skill_tree', 'nav_training') },
    { href: "#tools", label: t('arsenal', 'nav_training') },
    { href: "#certifications", label: t('achievements', 'nav_training') },
  ];

  const careersLinks = [
    { href: "#careers", label: t('careers', 'nav_careers') },
    { href: "#partners", label: t('guilds', 'nav_careers') },
  ];
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 max-w-7xl items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
            <Button variant="ghost" asChild className="hover:bg-[#814BFD] hover:text-white transition-shadow hover:shadow-[0_0_20px_2px_#814BFD]"><Link href="#home">{t('home', 'nav_main')}</Link></Button>
            <DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="hover:bg-[#814BFD] hover:text-white transition-shadow hover:shadow-[0_0_20px_2px_#814BFD]">{t('training', 'nav_main')} <ChevronDown className="ml-1 h-4 w-4" /></Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px] bg-card">
                <div className="flex flex-col h-full">
                  <div className="flex justify-between items-center mb-6">
                      <Link href="#home" className="flex items-center gap-2 group">
                          <Image src="/LOGOIC2.png" alt="MAD Academy Logo" width={180} height={38} />
                      </Link>
                  </div>
                   <nav className="flex flex-col gap-2">
                    <SheetClose asChild>
                      <Link href="#home" className="text-lg transition-colors hover:text-primary py-4 border-b">{t('home', 'nav_main')}</Link>
                    </SheetClose>
                    <Accordion type="multiple" className="w-full">
                      ...
                      <AccordionItem value="training">
                        <AccordionTrigger className="text-lg hover:no-underline">{t('training', 'nav_main')}</AccordionTrigger>
                        <AccordionContent>
                          <div className="flex flex-col gap-2 pl-4">
